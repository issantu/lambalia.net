              ...
              {user ? (
                <div className="flex items-center space-x-3">
                  <span className="text-sm text-gray-700 flex items-center space-x-1.5">
                    ...
                    <span className="hidden md:inline">{t('common.welcome')}</span>
                  </span>
                  <button 
                    onClick={logout}
...
// Enhanced Grocery Store Ad Banner - Automated Revenue Generation
const ExternalAdBanner = ({ placement = "header", size = "728x90", ingredients = [] }) => {
  const [adContent, setAdContent] = useState(null);
  const [loading, setLoading] = useState(true);
  const { user } = useAuth();
  
  useEffect(() => {
    const loadGroceryAds = async () => {
      if (!user) {
        setLoading(false);
        return;
      }
      ...
      try {
        // Get user's postal code or default
        ...
        if (response.ok) {
          ...
          if (ads && ads.length > 0) {
            // Select first ad (they're already sorted by priority)
            setAdContent(ads[0]);
          }
        }
      } catch (error) {
        console.log('Grocery ad loading error:', error);
      }
    ...
    loadGroceryAds();
  }, [placement, size, ingredients, user]);
    ...
    return (
      <div className={`external-ad-container bg-gray-50 border border-gray-200 rounded-lg p-4 text-center ${
        ...
        <div className="flex items-center justify-center h-full">
          <div className="text-sm text-gray-500">Loading grocery deals...</div>
        </div>
      </div>
    );
  }
  ...
  if (!adContent) {
    // Fallback to generic grocery ad
    return (
      <div className={`external-ad-container bg-gradient-to-r from-green-50 to-blue-50 border border-green-200 rounded-lg p-4 text-center ${
        size === '728x90' ? 'h-24' : size === '300x250' ? 'h-64 w-80' : 'h-16'
      } mx-auto mb-4 cursor-pointer hover:shadow-md transition-shadow`}>
        <div className="flex flex-col items-center justify-center h-full">
          <div className="text-xs text-green-600 mb-1">Advertisement</div>
          <div className="text-sm font-medium text-gray-700">
            🛒 Find Fresh Ingredients at Local Grocery Stores
          </div>
          <div className="text-xs text-gray-500 mt-1">
            Walmart • Kroger • Target • H-Mart • More
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div 
      className={`external-ad-container bg-white border border-gray-200 rounded-lg p-4 text-center ${
        size === '728x90' ? 'h-24' : size === '300x250' ? 'h-64 w-80' : 'h-16'
      } mx-auto mb-4 cursor-pointer hover:shadow-lg transition-all duration-200 transform hover:scale-105`}
      ...
      <div className="flex items-center justify-center h-full space-x-4">
        ...
        <div className="flex-shrink-0">
          <img 
            ...
            alt={adContent.chain_id}
            className="h-8 w-auto object-contain"
            onError={(e) => {
              e.target.src = `https://via.placeholder.com/60x30/4F46E5/white?text=${adContent.chain_id.toUpperCase()}`;
            }}
          />
        </div>
        ...
        <div className="flex-1 text-left">
          <div className="text-xs text-blue-600 mb-1">
            {adContent.store_location ? `${adContent.store_location.distance_km}km away` : 'Nearby'}
          </div>
        ...
        {/* CTA Button */}
        <div className="flex-shrink-0">
          <div className="bg-blue-600 text-white px-3 py-1 rounded text-xs font-medium hover:bg-blue-700 transition-colors">
            {adContent.cta_text || 'Shop Now'}
          </div>
        </div>
      </div>
      
      {/* Special Deals Display */}
      {adContent.weekly_deals && adContent.weekly_deals.length > 0 && (
        <div className="mt-2 pt-2 border-t border-gray-100">
          <div className="flex justify-center space-x-4 text-xs text-green-600">
            {adContent.weekly_deals.slice(0, 3).map((deal, index) => (
              <span key={index} className="bg-green-50 px-2 py-1 rounded">
                {deal.description}
              </span>
            ))}
          </div>
        ...
        <section>
          ...
          <div className="space-y-6">
            <div className="bg-gradient-to-r from-green-50 to-blue-50 p-6 rounded-xl">
              ...
              <a href="mailto:sales@lambalia.net" className="text-green-600 font-medium">
                sales@lambalia.net
              </a>
            </div>
